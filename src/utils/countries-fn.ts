...
export const findCountryByCode = (code?: CountryCode) =>
  allCountries.find((country) => country.iso2 === code);
...
export const getCountryByDialCode = (phoneNumber: string) => {
  ...
  return allCountries.find((country) =>
    country.dialCode.startsWith(`+${tel.replace(" ", "")}`),
  );
};

export const getCountriesByRegions = (
  ...
  return allCountries.filter((country) =>
    regions.map((region) => country.regions.includes(region)).some((el) => el),
  );
};
...
export const getCountryList = (
  ...
  if (preferredCountries) {
    return preferredCountries.map((prefCountry) =>
      findCountryByCode(prefCountry),
    );
  }
