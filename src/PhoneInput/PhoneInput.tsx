...
import { PhoneNumber, ICountry, IPhoneInputProps, IsoCode } from '../types';
import './styles.scss';
...
export const PhoneInput: FC<IPhoneInputProps> = ({
  ...
  useEffect(() => {
    ...
    if (showCountries && iso2 && iso2 !== 'NO_FLAG' && countryList.current && activeCountry.current) {
  ...
  useEffect(() => {
    ...
    if (initialValue) {
      const formated = formatNumber(initialValue, format, iso2);
      const metaData = findPhoneNumbersInText(initialValue);

      setCountry(findCountryBy('iso2', metaData[0].number.country || 'NO_FLAG'));
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement>): void => {
    ...
    const formated = formatNumber(value, format, iso2);
      ...
      {isInternational && (
        <div onClick={!isMobile ? handleToggleList : undefined} className="flag-wrapper" role="none">
          {iso2 === 'NO_FLAG' ? (
            ...
            <FlagIcon countryCode={iso2} svg aria-label={name} title={iso2} style={{ width: '20px', height: '20px' }} />
          )}
          {isMobile && (
